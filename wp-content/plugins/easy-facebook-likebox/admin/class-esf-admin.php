	...
	class ESF_Admin {
		...
		function esf_change_module_status() {
			...
			if ( isset( $status_updated ) ) {
				...
				echo wp_send_json_success( __( ucfirst( $module_name ) . $status . ' Successfully', $Feed_Them_All->fta_slug ) );
				wp_die();
			...
			if ( $diff_intrval >= 6 && get_site_option( 'fta_supported' ) != "yes" ) { ?>
                ...
                <div style="position:relative;padding-right:80px;background: #fff;"
                    ...
                    <p><?php esc_html_e( "Awesome, you have been using ", 'easy-facebook-likebox' ); ?>
                        <b><?php esc_html_e( "Easy Social Feed", 'easy-facebook-likebox' ); ?></b>
						<?php esc_html_e( "for more than 1 week. May I ask you to give it a", 'easy-facebook-likebox' ); ?>
                        <b><?php esc_html_e( "5-star", 'easy-facebook-likebox' ); ?></b>
						<?php esc_html_e( "rating on Wordpress? This will help to spread its popularity and to make it plugin a better one. Your help is much appreciated.", 'easy-facebook-likebox' ); ?>
                    </p>
		...
		public function esf_access_token_valid() {
			...
			if ( $data_access_expires_at <= $current_timestamp ) {
				...
				$return_arr = [
					...
					'error_message' => __( "Attention! Data access to the current access token in expired. Please reauthenticate the app.", 'easy-facebook-likebox' ),
				];
			...
			if ( ( $expires_at > 0 ) && ( $expires_at <= $current_timestamp ) ) {
				...
				$return_arr = [
					...
					'error_message' => __( "Attention! Access token in expired. Please reauthenticate the app.", 'easy-facebook-likebox' ),
				];
