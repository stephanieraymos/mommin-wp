...
if ( ! function_exists( 'get_css3_animations' ) ) {
	function get_css3_animations() {
		...
		return apply_filters( 'efbl_css3_effects', $css3_effects );
...
if ( ! function_exists( 'efbl_check_reaction' ) ) {
	function efbl_check_reaction( $needle, $array, $filter = null ) {
		...
		foreach ( $array as $efbl_reaction ):
			...
			if ( $needle == $efbl_reaction['type'] ): $efbl_reaction_count ++;
				$efbl_reaction_array['data'][] = $efbl_reaction; endif;
		endforeach;
...
if ( ! function_exists( 'ecff_makeClickableLinks' ) ) {
	function ecff_makeClickableLinks(
		$value, $protocols = [
		'http',
		'mail',
		'https',
